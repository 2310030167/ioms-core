    ...
    <div className="bg-[#070510]/40 border border-purple-500/10 shadow-[0_24px_50px_rgba(0,0,0,0.7)] rounded-xl p-6 backdrop-blur-md space-y-4">
      ...
      <div className="w-full overflow-x-auto">
        <table className="w-full text-left text-xs min-w-[650px]">
          <thead>
            <tr className="text-zinc-500 border-b border-zinc-800/40 text-[10px] uppercase tracking-wider"><th className="pb-3 font-medium">Timestamp</th><th className="pb-2 font-medium">Operator Context</th><th className="pb-2 font-medium">Action</th><th className="pb-2 font-medium">Target Table</th><th className="pb-2 font-medium">Telemetry Mapping</th></tr>
          </thead>
          <tbody className="divide-y divide-zinc-900/30 text-zinc-300 font-mono text-[11px]">
            {logs.length === 0 ? <tr><td colSpan={5} className="py-6 text-center text-zinc-600">NO APPLICATION CORE EVENTS REPORTED</td></tr> : logs.map(l => (
              <tr key={l.id} className="hover:bg-purple-500/[0.02] transition-colors duration-300 ease-out">
                ...
                <td className="py-3 text-zinc-500 max-w-xs truncate" title={l.details}>{l.details || "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
