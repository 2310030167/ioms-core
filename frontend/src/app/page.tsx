  ...
  return (
    <div className="flex h-screen w-screen bg-[#050409] text-zinc-300 font-sans selection:bg-purple-950 overflow-hidden relative">
      ...
      <main className="flex-1 flex flex-col bg-transparent overflow-hidden relative z-10">
        <header className="h-14 border-b border-purple-900/30 flex items-center justify-between px-4 md:px-6 flex-shrink-0 backdrop-blur-md bg-[#05040B]/40">
          <div className="flex items-center gap-3">
            <button onClick={() => setMo(true)} className="p-1.5 text-zinc-400 hover:text-white lg:hidden rounded-lg bg-purple-900/40 border border-purple-800/30"><Menu className="w-4 h-4" /></button>
        ...
        <section className="flex-1 overflow-y-auto p-4 md:p-6">
          <div className="max-w-6xl mx-auto space-y-6">
            ...
            {tb === "dash" && (
              <div className="space-y-6">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                  <div className="bg-purple-900/10 border border-purple-800/20 shadow-[0_12px_32px_rgba(0,0,0,0.4)] border-b-[5px] border-purple-900/60 p-4 md:p-6 lg:col-span-2 overflow-x-auto backdrop-blur-sm rounded-2xl">
                    <div className="flex items-center justify-between pb-3 mb-5 border-b border-purple-800/20 min-w-[500px]"><h3 className="text-xs font-bold text-purple-400/70 uppercase tracking-wider">Task Status Tracker</h3><div className="text-purple-500/60 font-mono text-[9px] tracking-widest">DATA_REFRESH_OK</div></div>
                    <div className="grid grid-cols-7 gap-3 min-w-[500px]">
                      {Object.entries(tM).map(([stVal, count]) => {
                        ...
                        return (
                          <div key={stVal} className="bg-purple-900/10 border border-purple-800/20 shadow-[0_10px_25px_rgba(0,0,0,0.4),inset_0_1px_1px_rgba(255,255,255,0.02)] border-b-[3px] border-purple-900/60 px-2 py-4 rounded-2xl text-center flex flex-col justify-between items-center hover:scale-[1.03] hover:border-purple-500/30 transition-all duration-200 group">
                            ...
                            <div className="w-5 bg-black/60 h-28 my-3 rounded-full relative flex items-end justify-center overflow-hidden border border-purple-800/20 shadow-[inset_0_2px_5px_rgba(0,0,0,0.7)]">
                  ...
                  <div className="bg-purple-900/10 border border-purple-800/20 shadow-[0_12px_30px_rgba(0,0,0,0.4)] border-b-[5px] border-purple-900/60 rounded-2xl p-4 md:p-5 backdrop-blur-sm">
                    <div className="pb-3 mb-3 border-b border-purple-800/20"><h3 className="text-xs font-bold text-purple-400/60 uppercase tracking-wider">Quick Templates</h3></div>
                    <div className="space-y-1.5 max-h-[168px] overflow-y-auto pr-0.5">
                      {bp.length === 0 ? <div className="text-center text-[11px] font-mono text-purple-500/40 py-6">NO TEMPLATES CONFIGURED</div> : bp.map(b => (
                        <div key={b.id} onClick={() => { if(rl !== "viewer") abp(b); }} className={`bg-purple-900/10 border border-purple-800/20 p-2 rounded-xl flex items-center justify-between gap-2 group transition-colors shadow-sm ${rl !== "viewer" ? "cursor-pointer hover:border-purple-800/60" : ""}`}>
                ...
                {rl === "operator" && (
                  <div className="bg-purple-900/10 border border-purple-800/20 shadow-[0_12px_32px_rgba(0,0,0,0.4)] border-b-[5px] border-purple-900/60 rounded-2xl p-5 backdrop-blur-sm">
                    <div className="flex items-center gap-2 pb-3 mb-4 border-b border-purple-800/20">
                    ...
                    <form onSubmit={sl} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-3">
                        <div>
                          ...
                          <textarea required rows={2} value={lf.cp} onChange={e => setLf({...lf, cp: e.target.value})} className="w-full bg-purple-900/10 border border-purple-800/20 rounded-xl px-3 py-2 text-xs text-zinc-100 focus:outline-none focus:border-purple-600 shadow-inner" placeholder="List items or features completed..." />
                        </div>
                        <div>
                          ...
                          <textarea value={lf.bl} onChange={e => setLf({...lf, bl: e.target.value})} className="w-full bg-purple-900/10 border border-purple-800/20 rounded-xl px-3 py-2 text-xs text-zinc-100 focus:outline-none focus:border-purple-600 shadow-inner" placeholder="Any blockers (or None)..." />
                        </div>
                      </div>
                      <div className="space-y-3 flex flex-col justify-between">
                        <div>
                          ...
                          <textarea required rows={2} value={lf.pl} onChange={e => setLf({...lf, pl: e.target.value})} className="w-full bg-purple-900/10 border border-purple-800/20 rounded-xl px-3 py-2 text-xs text-zinc-100 focus:outline-none focus:border-purple-600 shadow-inner" placeholder="What are you working on tomorrow?..." />
                        </div>
                        <div className="flex gap-3 items-end">
                          <div className="flex-1">
                            <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-1">Hours Worked</label>
                            <input required type="number" step="0.5" min="0.5" max="24" value={lf.hr} onChange={e => setLf({...lf, hr: e.target.value})} className="w-full bg-purple-900/10 border border-purple-800/20 rounded-xl px-3 py-2 text-xs text-zinc-100 focus:outline-none focus:border-purple-600 shadow-inner" placeholder="8" />
                          </div>
            ...
            {tb === "clients" && (rl === "admin" || rl === "accounts") && (
              <div className="bg-purple-900/10 border border-purple-800/20 shadow-[0_12px_32px_rgba(0,0,0,0.4)] border-b-[5px] border-purple-900/60 rounded-2xl p-5 backdrop-blur-sm space-y-4">
                ...
                <div className="w-full overflow-x-auto">
                  <table className="w-full text-left text-xs min-w-[500px]">
                    <thead><tr className="text-purple-400/50 border-b border-purple-800/20 font-bold uppercase tracking-wider"><th className="pb-2">Client Name</th><th className="pb-2">Corporate Entity</th><th className="pb-2">Email Address</th><th className="pb-2 text-right">Actions</th></tr></thead>
                    <tbody className="divide-y divide-purple-900/10">
                      {cl.length === 0 ? <tr><td colSpan={4} className="py-4 text-center text-purple-500/40 font-mono">NO CUSTOMER SHEETS INDEXED</td></tr> : cl.map(c => (
                        <tr key={c.id} className="text-zinc-300 hover:bg-purple-900/20 transition-colors">
                  ...
                  <table className="w-full text-left text-xs min-w-[600px]">
                    ...
                    <tbody className="divide-y divide-purple-900/10">
                      {fi.length === 0 ? <tr><td colSpan={6} className="py-4 text-center text-purple-500/40 font-mono">NO TRANSFERS AUDITED YET</td></tr> : fi.map(f => (
                        <tr key={f.id} className="text-zinc-300 hover:bg-purple-900/20 transition-colors">
                          <td className="py-3 font-semibold text-white">{f.client_name}</td>
                          <td className="py-3"><span className={`px-2 py-0.5 text-[10px] rounded border font-mono font-bold ${f.type === "Invoice" ? "bg-purple-900 text-purple-300 border-purple-800/40" : f.type === "Expense" ? "bg-rose-950/40 text-rose-400 border-rose-900/30" : "bg-zinc-900 text-zinc-400 border-zinc-800"}`}>{f.type.toUpperCase()}</span></td>
            ...
            {tb === "reports" && rl === "admin" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-purple-900/10 border border-purple-800/20 shadow-[0_12px_32px_rgba(0,0,0,0.4)] border-b-[5px] border-purple-900/60 rounded-2xl p-5 backdrop-blur-sm h-[560px] flex flex-col">
                  <h3 className="text-xs font-bold text-purple-400/70 uppercase tracking-wider pb-3 border-b border-purple-800/20 mb-4 flex-shrink-0">Select Team Member</h3>
                  <div className="space-y-2 overflow-y-auto flex-1 pr-1">
                    {us.map(u => (
                      <div key={u.id} onClick={() => setSu(u.email)} className={`p-4 rounded-2xl border text-left cursor-pointer transition-all duration-200 ${su === u.email ? "bg-purple-900/40 border-purple-500/60 text-white shadow-[0_8px_20px_rgba(124,58,237,0.15),inset_0_1px_2px_rgba(255,255,255,0.05)] border-b-[4px] border-purple-600" : "bg-purple-900/5 border border-purple-800/20 text-zinc-400 hover:border-purple-800/60 hover:text-zinc-200 border-b-[4px] border-purple-900"}`}>
                ...
                <div className="bg-purple-900/10 border border-purple-800/20 shadow-[0_12px_32px_rgba(0,0,0,0.4)] border-b-[5px] border-purple-900/60 rounded-2xl p-5 backdrop-blur-sm h-[560px] flex flex-col">
                  <h3 className="text-xs font-bold text-purple-400/70 uppercase tracking-wider pb-3 border-b border-purple-800/20 mb-4 flex-shrink-0">Activity Breakdown</h3>
                  {su ? (
                    <div className="flex-1 flex flex-col overflow-hidden">
                      <div className="bg-purple-900/10 border border-purple-800/20 p-4 rounded-2xl mb-4 flex-shrink-0 shadow-inner">
                        ...
                        <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t border-purple-800/20 text-center">
                          <div>
                      ...
                      <div className="flex-1 overflow-y-auto space-y-4 pr-1">
                        <div>
                          ...
                          {wl.filter(w => w.email === su).length === 0 ? (
                            ...
                            <div key={w.id} className="bg-purple-900/5 border border-purple-800/10 rounded-xl p-3.5 mb-2 text-xs space-y-1.5 shadow-sm">
                              <div className="flex justify-between items-center text-[10px] font-mono text-zinc-500 border-b border-purple-800/20 pb-1.5">
                        ...
                        <div>
                          ...
                          {ln.filter(l => l.email === su).length === 0 ? (
                            ...
                            <div key={l.id} className="bg-purple-900/5 border border-purple-800/10 rounded-xl p-2.5 flex items-center justify-between gap-4 shadow-sm mb-1.5">
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-6 border-2 border-dashed border-purple-800/20 rounded-2xl">
            ...
            {tb !== "reports" && tb !== "clients" && tb !== "finance" && (
              <div className="bg-purple-900/10 border border-purple-800/20 shadow-[0_12px_32px_rgba(0,0,0,0.4)] border-b-[5px] border-purple-900/60 rounded-2xl p-4 md:p-5 overflow-hidden backdrop-blur-sm">
                {tb === "team" ? (
                  <div className="space-y-8">
                    <div>
                      <div className="flex items-center justify-between mb-4 pb-2 border-b border-purple-800/20">
                              ...
                              <tr key={u.id} className="text-zinc-300 hover:bg-purple-950/5 transition-colors">
                                ...
                                <td className="py-3">
                                  <select disabled={u?.id === au?.id} value={u?.role} onChange={async (e) => {
                                    ...
                                    await gd();
                                  }} className="bg-purple-900/50 text-xs border border-purple-800/30 px-2.5 py-1 rounded-lg text-zinc-200 font-semibold focus:outline-none focus:border-purple-600 shadow-inner">
                    ...
                    <div>
                      ...
                      <div className="w-full overflow-x-auto">
                        <table className="w-full text-left text-xs min-w-[500px]">
                          <thead><tr className="text-purple-400/50 border-b border-purple-800/20 font-bold uppercase tracking-wider"><th className="pb-2">User Email</th><th className="pb-2">Action Status</th><th className="pb-2 text-right">Date & Time</th></tr></thead>
                          <tbody className="divide-y divide-purple-900/40">
                            {ln.length === 0 ? <tr><td colSpan={3} className="py-4 text-center font-semibold text-purple-500/30">NO ACTIVITY TRACKED YET</td></tr> : ln.map(l => (
                              <tr key={l.id} className="text-zinc-300 hover:bg-purple-900/5 transition-colors">
                  ...
                  <div>
                    ...
                    {ld ? <div className="text-center py-8 font-mono text-purple-400/40 animate-pulse tracking-wide text-xs">LOADING DATA ENTRIES...</div> : tb === "dash" || tb === "proj" ? (
                      pj.length === 0 ? <div className="text-center py-8 text-xs text-purple-400/40 font-mono">NO PROJECTS FOUND</div> : (
                        <div className="w-full overflow-x-auto">
                          <table className="w-full text-left text-xs min-w-[550px]">
                            <thead><tr className="text-purple-400/50 border-b border-purple-800/20 font-mono uppercase tracking-wider"><th className="pb-2">Project Name</th><th className="pb-2">Status State</th>{rl === "admin" && <th className="pb-2 text-right">Remove</th>}</tr></thead>
                            <tbody className="divide-y divide-purple-900/10">
                              {pj.map(p => (
                                <tr key={p.id} className="text-white hover:bg-purple-900/5 transition-colors">
                      ...
                      <div className="w-full overflow-x-auto">
                        <table className="w-full text-left text-xs min-w-[550px]">
                          <thead><tr className="text-purple-400/50 border-b border-purple-800/20 font-bold uppercase tracking-wider"><th className="pb-2">Task Title</th><th className="pb-2">Assigned Employee</th><th className="pb-2">Progress Status</th>{rl === "admin" && <th className="pb-2 text-right">Remove</th>}</tr></thead>
                          <tbody className="divide-y divide-purple-900/10">
                            {tk.map(t => (
                              <tr key={t.id} className="text-white hover:bg-purple-900/5 transition-colors">
      ...
      {cf && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-in fade-in duration-150">
          <div className="bg-[#0D0A16] border border-purple-900/40 rounded-2xl max-w-sm w-full p-5 shadow-2xl">
            ...
            <p className="text-xs text-zinc-400 leading-relaxed">Are you sure you want to permanently delete <span className="text-purple-300 font-mono bg-purple-900/40 px-1 py-0.5 rounded border border-purple-900/30">"{cf.name}"</span>?</p>
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setCf(null)} className="px-3.5 py-1.5 text-xs font-semibold bg-purple-900/40 border border-purple-900/40 hover:bg-purple-900/60 text-zinc-300 rounded-xl transition-colors">Cancel</button>
