...
export default function Home() {
  ...
  const lk = [
    ...
    ...(rl === "admin" ? [{ id: "team", label: "Team Management", icon: Users }] : [])
  ];
      ...
      <div className="min-h-screen w-screen bg-[#07050F] flex flex-col items-center justify-center font-sans p-4 relative overflow-hidden selection:bg-purple-950/40">
        ...
        <div className="w-full max-w-[380px] bg-purple-950/30 border border-purple-800/20 rounded-3xl p-8 backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.6),inset_0_2px_4px_rgba(255,255,255,0.05)] border-b-4 border-purple-900/60 relative z-10">
          <div className="flex flex-col items-center mb-8">
            <div className="w-12 h-12 rounded-2xl bg-purple-900/20 border border-purple-700/30 flex items-center justify-center text-purple-400 mb-4 shadow-[0_8px_16px_rgba(147,51,234,0.2)]">
          ...
          <form onSubmit={li} className="space-y-4">
            <div>
              <label className="block text-[11px] font-medium text-zinc-400 tracking-wide mb-1.5">Email Address</label>
              <input required type="email" value={fm.email} onChange={e => setFm({...fm, email: e.target.value})} className="w-full bg-purple-950/20 border border-purple-900/40 rounded-xl px-3.5 py-2.5 text-sm text-zinc-100 focus:outline-none focus:border-purple-500/60 transition-colors shadow-[inset_0_2px_4px_rgba(0,0,0,0.4)]" placeholder="yourname@domain.com" />
            </div>
            <div>
              <label className="block text-[11px] font-medium text-zinc-400 tracking-wide mb-1.5">Password</label>
              <input required type="password" value={fm.password} onChange={e => setFm({...fm, password: e.target.value})} className="w-full bg-purple-950/20 border border-purple-900/40 rounded-xl px-3.5 py-2.5 text-sm text-zinc-100 focus:outline-none focus:border-purple-500/60 transition-colors shadow-[inset_0_2px_4px_rgba(0,0,0,0.4)]" placeholder="••••••••••••" />
            </div>
  ...
  return (
    <div className="flex h-screen w-screen bg-[#05040B] text-zinc-300 font-sans selection:bg-purple-950 overflow-hidden relative">
      ...
      {ts && (
        <div className="fixed top-5 right-5 z-50 max-w-xs w-[calc(100vw-2rem)] bg-purple-950/40 border border-purple-800/40 backdrop-blur-md rounded-xl p-3.5 shadow-[0_10px_30px_rgba(0,0,0,0.5)] flex items-start gap-2.5 animate-in fade-in slide-in-from-top-3">
      ...
      <aside className={`fixed inset-y-0 left-0 w-60 bg-[#0A0713]/90 border-r border-purple-950/50 backdrop-blur-xl flex flex-col justify-between z-40 transition-transform duration-300 lg:static lg:translate-x-0 shadow-[4px_0_24px_rgba(0,0,0,0.4)] ${mo ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="p-5 flex flex-col h-full overflow-y-auto">
          <div className="flex items-center justify-between mb-6 px-1">
            <div className="flex items-center gap-2.5"><Activity className="w-5 h-5 text-purple-400 stroke-[1.5]" /><span className="font-bold text-sm text-zinc-100 tracking-tight">IOMS Workspace</span></div>
            <button onClick={() => setMo(false)} className="lg:hidden p-1 text-zinc-500 hover:text-white"><X className="w-4 h-4" /></button>
          </div>
          <div className="mb-4 p-3 bg-purple-950/20 border border-purple-900/30 rounded-xl backdrop-blur-sm shadow-[inset_0_1px_2px_rgba(255,255,255,0.05)] border-b-2 border-purple-900/40">
            <div className="flex items-center gap-2 mb-2.5">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[9px] font-mono text-purple-400/80 uppercase tracking-wider">{rl}</p>
              </div>
            </div>
            <button onClick={lo} className="w-full flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-[10px] font-bold text-purple-300 bg-purple-950/40 border border-purple-900/40 hover:bg-purple-900/60 hover:text-white border-b-2 border-purple-900/80 active:border-b-0 active:translate-y-px transition-all"><LogOut className="w-3 h-3" /> Sign Out</button>
          </div>
          <nav className="space-y-1 flex-1">
            {lk.map(l => (
              <button key={l.id} onClick={() => { setTb(l.id); setMo(false); }} className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-semibold transition-all ${tb === l.id ? "bg-purple-950/40 text-purple-300 shadow-[inset_0_1px_2px_rgba(0,0,0,0.6)] border border-purple-900/30" : "text-zinc-500 hover:bg-purple-950/10 hover:text-zinc-300"}`}>
      ...
      <main className="flex-1 flex flex-col bg-transparent overflow-hidden">
        <header className="h-14 border-b border-purple-950/30 flex items-center justify-between px-4 md:px-6 flex-shrink-0 backdrop-blur-md bg-[#05040B]/40">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xs font-bold tracking-wide uppercase text-purple-400/80">{tb === "team" ? "Team Roles" : tb} Section</h1>
          </div>
          ...
          <div className="max-w-6xl mx-auto space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[{ t: "Total Projects", v: ld ? "..." : st.pipeline }, { t: "Total Tasks", v: ld ? "..." : st.tasks }, { t: "Your Account Role", v: ld ? "..." : rl.toUpperCase() }].map((c, i) => (
                <div key={i} className="bg-purple-950/10 border border-purple-900/20 shadow-[0_8px_20px_rgba(0,0,0,0.3),inset_0_1px_1px_rgba(255,255,255,0.05)] border-b-4 border-purple-950/40 p-4 rounded-2xl backdrop-blur-sm"><h3 className="text-[10px] font-bold text-purple-400/50 uppercase tracking-wider">{c.t}</h3><p className="text-2xl font-black text-zinc-100 mt-1 tracking-tight">{c.v}</p></div>
              ))}
            </div>
            
            {tb === "dash" && (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="bg-purple-950/10 border border-purple-900/20 shadow-[0_8px_20px_rgba(0,0,0,0.3)] border-b-4 border-purple-950/40 rounded-2xl p-4 md:p-5 lg:col-span-2 overflow-x-auto backdrop-blur-sm">
                  ...
                  <div className="grid grid-cols-7 gap-2 min-w-[500px]">
                    {Object.entries(tM).map(([stVal, count]) => (
                      <div key={stVal} className="bg-purple-950/5 border border-purple-900/10 p-2.5 rounded-xl text-center flex flex-col justify-between shadow-[inset_0_1px_2px_rgba(0,0,0,0.5)]">
                        <span className="text-[9px] text-purple-400/50 uppercase truncate font-bold font-mono tracking-tighter block">{stVal.replace("_", " ")}</span>
                        <div className="w-full bg-purple-950/20 h-16 mt-2 rounded-lg relative flex items-end justify-center overflow-hidden border border-purple-900/20"><div style={{ height: `${Math.min(count * 20, 100)}%` }} className="w-2.5 bg-gradient-to-t from-purple-600 to-purple-400 rounded-t shadow-[0_0_10px_rgba(168,85,247,0.4)] transition-all duration-500"></div></div>
                        <span className="text-xs font-mono font-bold text-white mt-1.5 block">{count}</span>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="bg-purple-950/10 border border-purple-900/20 shadow-[0_8px_20px_rgba(0,0,0,0.3)] border-b-4 border-purple-950/40 rounded-2xl p-4 md:p-5 backdrop-blur-sm">
            ...
            <div className="bg-purple-950/10 border border-purple-900/20 shadow-[0_12px_32px_rgba(0,0,0,0.4)] border-b-4 border-purple-950/40 rounded-2xl p-4 md:p-5 overflow-hidden backdrop-blur-sm">
              {tb === "team" ? (
                ...
                <div>
                  ...
                  {ld ? <div className="text-center py-8 font-mono text-purple-400/40 animate-pulse tracking-wide text-xs">LOADING DATA ENTRIES...</div> : tb === "dash" || tb === "proj" ? (
                    pj.length === 0 ? <div className="text-center py-8 text-xs text-purple-400/40 font-mono">NO PROJECTS FOUND</div> : (
                      <div className="w-full overflow-x-auto">
                        <table className="w-full text-left text-xs min-w-[550px]">
                          <thead><tr className="text-purple-400/50 border-b border-purple-900/20 font-mono uppercase tracking-wider"><th className="pb-2 font-medium">Project Name</th><th className="pb-2 font-medium">Status State</th>{rl === "admin" && <th className="pb-2 text-right font-medium">Remove</th>}</tr></thead>
                          <tbody className="divide-y divide-purple-900/10">
                            {pj.map(p => (
                              <tr key={p.id} className="text-white hover:bg-purple-950/5 transition-colors">
                                ...
                                <td className="py-3.5">
                                  <select disabled={rl === "viewer"} value={p.status} onChange={async (e) => {
                                    ...
                                    await gd();
                                  }} className="bg-purple-950/50 text-xs border border-purple-900/30 px-2 py-1 rounded-lg text-white focus:outline-none focus:border-purple-600 disabled:opacity-50">
                                    {["New", "Planning", "Development", "Testing", "Completed"].map(s => <option key={s} value={s}>{s}</option>)}
                                  </select>
                                </td>
                                {rl === "admin" && <td className="py-3.5 text-right"><button onClick={() => setCf({ id: p.id, type: "proj", name: p.name })} className="text-purple-400/40 hover:text-rose-400 p-1.5 rounded transition-colors"><Trash2 className="w-4 h-4" /></button></td>}
                              </tr>
                            ))}
                          </tbody>
                        </table>
                    ...
                    tk.length === 0 ? <div className="text-center py-10 text-xs text-purple-400/40 font-mono">NO TASKS FOUND</div> : (
                      <div className="w-full overflow-x-auto">
                        <table className="w-full text-left text-xs min-w-[550px]">
      ...
      {cf && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-in fade-in duration-150">
          <div className="bg-zinc-950 border border-purple-900/40 rounded-2xl max-w-sm w-full p-5 shadow-2xl">
            ...
            <p className="text-xs text-zinc-400 leading-relaxed">Are you sure you want to permanently delete <span className="text-zinc-200 font-bold bg-zinc-900 px-1 py-0.5 rounded border border-purple-955/30">"{cf.name}"</span> from the database layer?</p>
