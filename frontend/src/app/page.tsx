...
export default function Home() {
  ...
  if (!au) {
    return (
      <div className="min-h-screen w-screen bg-[#000000] flex flex-col items-center justify-center font-sans p-4 relative overflow-hidden selection:bg-purple-950/40">
        ...
        <div className="w-full max-w-[380px] bg-[#050508]/60 border border-zinc-800/60 rounded-3xl p-8 backdrop-blur-xl shadow-2xl relative z-10">
          <div className="flex flex-col items-center mb-8">
            ...
            <h2 className="text-2xl font-bold text-white tracking-tight text-center">IOMS Central</h2>
            <p className="text-[11px] text-purple-400 font-semibold uppercase tracking-wider mt-1">Authorized Access Only</p>
          </div>
          <form onSubmit={li} className="space-y-4">
            <div>
              <label className="block text-[11px] font-semibold text-zinc-400 tracking-wide mb-1.5">Email Address</label>
              <input required type="email" value={fm.email} onChange={e => setFm({...fm, email: e.target.value})} className="w-full bg-[#000000] border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-purple-500/60 transition-colors shadow-inner" placeholder="name@domain.com" />
            </div>
            <div>
              <label className="block text-[11px] font-semibold text-zinc-400 tracking-wide mb-1.5">Password</label>
              <input required type="password" value={fm.password} onChange={e => setFm({...fm, password: e.target.value})} className="w-full bg-[#000000] border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-purple-500/60 transition-colors shadow-inner" placeholder="••••••••••••" />
            </div>
            ...
            <button type="submit" className="w-full bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold py-3.5 rounded-xl tracking-wide mt-2 shadow-lg transition-all active:scale-[0.99]">
              SIGN IN
            </button>
          </form>
        </div>
  ...
  return (
    <div className="flex h-screen w-screen bg-[#000000] text-zinc-100 font-sans selection:bg-purple-950 overflow-hidden relative">
      <AmbientCanvas />
      ...
      <aside className={`fixed inset-y-0 left-0 w-64 bg-[#050508] border-r border-zinc-900 flex flex-col justify-between z-40 transition-transform duration-300 lg:static lg:translate-x-0 shadow-[4px_0_30px_rgba(0,0,0,0.5)] ${mo ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="p-6 overflow-y-auto flex-1 relative z-10">
          <div className="flex items-center justify-between mb-8 px-1">
            <div className="flex items-center gap-2.5"><Activity className="w-5 h-5 text-purple-400 stroke-[1.5]" /><span className="font-bold text-sm tracking-tight text-white">IOMS Central</span></div>
          ...
          <div className="mb-6 p-4 bg-[#0A0A0F] border border-zinc-800 rounded-2xl shadow-inner">
            ...
            <button onClick={lo} className="w-full flex items-center justify-center gap-1.5 py-2 rounded-xl text-[11px] font-bold text-purple-300 bg-purple-950/30 border border-purple-900/40 hover:bg-purple-900/50 hover:text-white transition-all"><LogOut className="w-3 h-3" /> Sign Out</button>
          </div>

          <nav className="space-y-1">
            {lk.map(l => (
              <button key={l.id} onClick={() => { setTb(l.id); setMo(false); }} className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-xs font-bold transition-all ${tb === l.id ? "bg-zinc-900 text-purple-400 shadow-sm border border-zinc-800" : "text-zinc-400 hover:bg-zinc-900/40 hover:text-zinc-200"}`}>
      ...
      <main className="flex-1 flex flex-col bg-transparent overflow-hidden relative z-10">
        <header className="h-14 border-b border-zinc-900/80 flex items-center justify-between px-6 flex-shrink-0 backdrop-blur-md bg-black/40">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xs font-bold tracking-wider uppercase text-zinc-400">{tb === "team" ? "Team Roles" : tb === "reports" ? "Individual Reports" : tb} Hub</h1>
          </div>
        ...
        <section className="flex-1 overflow-y-auto p-6">
          <div className="max-w-6xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                { t: "Total Projects", v: ld ? "..." : st.pipeline },
                { t: "Total Tasks", v: ld ? "..." : st.tasks },
                { t: "Your Account Role", v: ld ? "..." : rl.toUpperCase() }
              ].map((c, i) => (
                <div key={i} className="bg-[#050508]/60 border border-zinc-800/80 p-5 rounded-2xl shadow-xl backdrop-blur-md relative overflow-hidden group hover:border-zinc-700 transition-colors">
                  <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">{c.t}</h3>
                  <p className="text-3xl font-bold text-white mt-1 tracking-tight">{c.v}</p>
                </div>
              ))}
            </div>
            ...
            {tb === "dash" && (
              <div className="space-y-6">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                  <div className="bg-[#050508]/60 border border-zinc-800/80 shadow-xl rounded-2xl p-6 lg:col-span-2 overflow-x-auto backdrop-blur-md">
                    <div className="flex items-center justify-between pb-3.5 mb-5 border-b border-zinc-900/60 min-w-[500px]">
                      <h3 className="text-xs font-bold text-zinc-300 uppercase tracking-wider">Task Status Tracker</h3>
                      <div className="text-zinc-600 font-mono text-[10px] tracking-widest">DATA_REFRESH_OK</div>
                    </div>
                    <div className="grid grid-cols-7 gap-4 min-w-[500px]">
                      {Object.entries(tM).map(([stVal, count]) => {
                        ...
                        return (
                          <div key={stVal} className="bg-[#0A0A0F]/60 border border-zinc-900/50 px-2 py-4 rounded-xl text-center flex flex-col justify-between items-center shadow-lg hover:border-purple-500/20 transition-all duration-200 group">
                            <span className="text-[10px] font-bold tracking-wide text-zinc-500 group-hover:text-purple-400 uppercase truncate w-full transition-colors font-mono">{stVal.replace("_", " ")}</span>
                            <div className="w-5 bg-[#000000] h-28 my-3 rounded-full relative flex items-end justify-center overflow-hidden border border-zinc-900 shadow-inner">
                              <div style={{ height: `${Math.max(pct, count > 0 ? 12 : 0)}%` }} className="w-full bg-gradient-to-t from-purple-700 via-purple-500 to-fuchsia-400 rounded-full transition-all duration-700 ease-out shadow-[0_0_15px_rgba(168,85,247,0.5)] relative">
                  ...
                  <div className="bg-[#050508]/60 border border-zinc-800/80 shadow-xl rounded-2xl p-5 backdrop-blur-md">
                    ...
                    <div className="space-y-1.5 max-h-[168px] overflow-y-auto pr-0.5">
                      {bp.length === 0 ? <div className="text-center text-[11px] font-mono text-zinc-600 py-6">NO TEMPLATES CONFIGURED</div> : bp.map(b => (
                        <div key={b.id} onClick={() => { if(rl !== "viewer") abp(b); }} className={`bg-[#0A0A0F]/40 border border-zinc-900/60 p-2.5 rounded-xl flex items-center justify-between gap-2 group transition-colors shadow-sm ${rl !== "viewer" ? "cursor-pointer hover:border-zinc-800" : ""}`}>
                ...
                {rl !== "admin" && (
                  <div className="bg-[#050508]/60 border border-zinc-800/80 shadow-xl rounded-2xl p-6 backdrop-blur-md">
                    ...
                    <form onSubmit={sl} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-4">
                        <div>
                          ...
                          <textarea required rows={2} value={lf.cp} onChange={e => setLf({...lf, cp: e.target.value})} className="w-full bg-black border border-zinc-800 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-purple-600 shadow-inner" placeholder="List items or features completed..." />
                        </div>
                        <div>
                          ...
                          <textarea value={lf.bl} onChange={e => setLf({...lf, bl: e.target.value})} className="w-full bg-black border border-zinc-800 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-purple-600 shadow-inner" placeholder="Any blockers (or None)..." />
                        </div>
                      </div>
                      <div className="space-y-4 flex flex-col justify-between">
                        <div>
                          ...
                          <textarea required rows={2} value={lf.pl} onChange={e => setLf({...lf, pl: e.target.value})} className="w-full bg-black border border-zinc-800 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-purple-600 shadow-inner" placeholder="What are you working on tomorrow?..." />
                        </div>
                        <div className="flex gap-4 items-end">
                          <div className="flex-1">
                            <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-1.5">Hours Worked</label>
                            <input required type="number" step="0.5" min="0.5" max="24" value={lf.hr} onChange={e => setLf({...lf, hr: e.target.value})} className="w-full bg-black border border-zinc-800 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-purple-600 shadow-inner" placeholder="8" />
                          </div>
            ...
            {tb === "reports" && rl === "admin" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-[#050508]/60 border border-zinc-800/80 shadow-xl rounded-2xl p-5 backdrop-blur-md h-[560px] flex flex-col">
                  ...
                  <div className="space-y-2 overflow-y-auto flex-1 pr-1">
                    {us.map(u => (
                      <div key={u.id} onClick={() => setSu(u.email)} className={`p-4 rounded-xl border text-left cursor-pointer transition-all duration-200 ${su === u.email ? "bg-purple-950/20 border-purple-500/60 text-white shadow-lg border-b-[4px] border-purple-600" : "bg-[#0A0A0F]/60 border-zinc-900 text-zinc-400 hover:border-zinc-800 hover:text-zinc-200 border-b-[4px] border-black"}`}>
                      ...
                      <div className="flex-1 overflow-y-auto space-y-4 pr-1">
                        <div>
                          ...
                          {wl.filter(w => w.email === su).length === 0 ? (
                            ...
                            <div key={w.id} className="bg-[#0A0A0F]/40 border border-zinc-900 p-3.5 mb-2 text-xs space-y-1.5 rounded-xl shadow-sm">
                        ...
                        <div>
                          ...
                          {ln.filter(l => l.email === su).length === 0 ? (
                            ...
                            <div key={l.id} className="bg-[#0A0A0F]/40 border border-zinc-900 rounded-xl p-2.5 flex items-center justify-between gap-4 shadow-sm mb-1.5">
            ...
            {tb !== "reports" && (
              <div className="bg-[#050508]/60 border border-zinc-800/80 shadow-xl rounded-2xl p-5 overflow-hidden backdrop-blur-md">
                {tb === "team" ? (
          ...
          <div className="bg-[#050508] border border-zinc-800 rounded-2xl max-w-md w-full overflow-hidden shadow-2xl">
            ...
            {md === "proj" ? (
              ...
              <form onSubmit={cu} className="p-5 space-y-4">
                ...
                <div><label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-1.5">Permission Level</label><select value={uf.rl} onChange={e => setUf({...uf, rl: e.target.value})} className="w-full bg-black border border-zinc-800 rounded-xl px-3 py-2.5 text-xs text-white focus:outline-none focus:border-purple-600 shadow-inner">{["admin", "operator", "viewer"].map(s => <option key={s} value={s}>{s.toUpperCase()}</option>)}</select></div>
                <button type="submit" className="w-full bg-purple-600 hover:bg-purple-500 font-bold text-white text-xs py-2.5 rounded-xl transition-all mt-2">Create Account</button>
              </form>
            )}
          </div>
        </div>
