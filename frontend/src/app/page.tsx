...
import { LayoutDashboard, FolderKanban, CheckSquare, Activity, Plus, X, LogIn, LogOut, Trash2, AlertTriangle, CheckCircle2, User, Copy, UserPlus, Users, Menu } from "lucide-react";
...
export default function Home() {
  ...
  const [mMo, setMMo] = useState(false); // Mobile Menu Open state
  ...
  if (!au) {
    return (
      <div className="min-h-screen w-full bg-[#06040A] flex items-center justify-center font-sans p-4 selection:bg-[#2C213D]">
        <div className="w-full max-w-sm bg-[#110E1A] border border-[#231C30] rounded-2xl p-6 md:p-8 shadow-2xl relative overflow-hidden">
  ...
  return (
    <div className="flex h-screen w-full bg-[#09070F] text-[#E4E6ED] font-sans selection:bg-[#2C213D] overflow-hidden relative">
      {ts && (
        <div className="fixed top-4 right-4 md:top-6 md:right-6 z-50 max-w-[calc(100vw-32px)] md:max-w-sm w-full bg-[#110E1A] border border-[#231C30] rounded-xl p-4 shadow-2xl flex items-start gap-3">
          {ts.type === "ok" ? <CheckCircle2 className="w-5 h-5 text-emerald-400 mt-0.5 min-w-[20px]" /> : <AlertTriangle className="w-5 h-5 text-rose-400 mt-0.5 min-w-[20px]" />}
      ...
      {/* Sidebar */}
      <aside className={`fixed md:static inset-y-0 left-0 z-50 w-64 bg-[#110E1A] border-r border-[#231C30] flex flex-col justify-between transform transition-transform duration-200 ease-in-out ${mMo ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
        <div className="p-4 md:p-6 overflow-y-auto">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3 px-2">
              <Activity className="w-6 h-6 text-[#8B5CF6]" />
              <span className="font-bold text-lg text-white">IOMS Core</span>
            </div>
            <button className="md:hidden text-[#6A6185]" onClick={() => setMMo(false)}>
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="mb-6 p-3 bg-[#171324] border border-[#231C30] rounded-lg">
            <div className="flex items-center gap-2.5 mb-2.5 p-1">
              <div className="w-8 h-8 rounded-lg bg-[#221B38] border border-[#362B4C] flex items-center justify-center text-[#8B5CF6] shrink-0"><User className="w-4 h-4" /></div>
          ...
          <nav className="space-y-1">
            {lk.map(l => (
              <button 
                key={l.id} 
                onClick={() => { setTb(l.id); setMMo(false); }} 
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium ${tb === l.id ? "bg-[#221B38] text-white border-l-2 border-[#8B5CF6]" : "text-[#A29DB8] hover:bg-[#171324]"}`}
              >
      ...
      {/* Main Content */}
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden bg-[#06040A]">
        <header className="h-16 bg-[#110E1A] border-b border-[#231C30] flex items-center px-4 md:px-8">
          <div className="flex items-center gap-3">
            <button className="md:hidden text-[#A29DB8] hover:text-white" onClick={() => setMMo(true)}>
        ...
        <section className="flex-1 overflow-y-auto p-4 md:p-8">
          <div className="max-w-7xl mx-auto space-y-4 md:space-y-6">
            <div className="grid grid-cols-2 gap-4 md:gap-6">
              {[{ t: "Pipeline", v: ld ? "..." : st.pipeline }, { t: "Active Tasks", v: ld ? "..." : st.tasks }].map((c, i) => (
                <div key={i} className="bg-[#110E1A] border border-[#231C30] p-4 md:p-6 rounded-xl">
                  <h3 className="text-[10px] md:text-xs text-[#6A6185] uppercase">{c.t}</h3>
                  <p className="text-2xl md:text-3xl font-bold text-white mt-1 md:mt-2">{c.v}</p>
                </div>
              ))}
            </div>
            ...
            {tb === "dash" && (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
                <div className="bg-[#110E1A] border border-[#231C30] rounded-xl p-4 md:p-6 lg:col-span-2 overflow-x-auto">
                  <div className="flex items-center justify-between border-b border-[#231C30]/40 pb-3 mb-4 min-w-[300px]">
                    <h3 className="text-xs font-bold text-white uppercase">Workload</h3>
                    <div className="text-emerald-400 font-mono text-[10px]">LIVE SYNC OK</div>
                  </div>
                  <div className="grid grid-cols-7 gap-2 md:gap-4 min-w-[400px]">
                    {Object.entries(tM).map(([stVal, count]) => (
                      <div key={stVal} className="bg-[#09070F] border border-[#231C30] p-2 md:p-3 rounded-lg text-center">
                        <span className="text-[9px] md:text-[10px] text-[#A29DB8] uppercase truncate block">{stVal}</span>
                        <div className="w-full bg-[#171324] h-12 md:h-16 mt-2 rounded relative flex items-end justify-center"><div style={{ height: `${Math.min(count * 20, 100)}%` }} className="w-2 md:w-3 bg-purple-500 rounded-sm"></div></div>
                        <span className="text-[10px] md:text-xs font-mono font-bold text-white mt-1 block">{count}</span>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-2 max-h-[150px] overflow-y-auto pr-1">
                    {bp.length === 0 ? <div className="text-center text-xs text-[#6A6185] py-4">No templates.</div> : bp.map(b => (
                      <div key={b.id} onClick={() => { if(rl !== "viewer") abp(b); }} className={`bg-[#09070F] border border-[#231C30] p-2 rounded flex items-center justify-between ${rl !== "viewer" ? "cursor-pointer" : ""}`}>
                        <span className="text-[11px] md:text-xs text-white truncate pr-2">{b.name}</span>
                        {(rl === "admin" || rl === "operator") && <button onClick={(e) => dbp(b.id, e)} className="text-[#6A6185] hover:text-rose-400 shrink-0"><Trash2 className="w-3 h-3" /></button>}
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="bg-[#110E1A] border border-[#231C30] rounded-xl p-4 md:p-6 overflow-hidden">
              {tb === "team" ? (
                <div className="overflow-x-auto">
                  <div className="flex items-center justify-between mb-4 pb-2 border-b border-[#231C30]/40 min-w-[400px]">
                    <h2 className="text-sm font-bold text-white uppercase">Role Configuration</h2>
                  </div>
                    ...
                    <tbody>
                      {us.map(u => (
                        <tr key={u.id} className="border-b border-[#231C30]/50 text-white">
                          <td className="py-2.5 font-mono text-[11px] md:text-xs truncate pr-4">{u.email || "Processing handshake..."}</td>
                          <td className="py-2.5 text-right">
                            <select disabled={u.id === au.id} value={u.role} onChange={async (e) => {
                              ...
                              await gd();
                            }} className="bg-[#171324] text-[11px] md:text-xs border border-[#362B4C] px-1.5 py-1 rounded text-white disabled:opacity-40">
                ...
                <div className="overflow-x-auto">
                  <div className="flex items-center justify-between mb-4 pb-2 border-b border-[#231C30]/40 min-w-[400px]">
                    ...
                    {tb !== "dash" && (rl === "admin" || rl === "operator") && <button onClick={() => setMd(tb)} className="bg-[#7C3AED] text-white text-[11px] md:text-xs px-3 py-1.5 rounded-lg flex items-center gap-1 shrink-0"><Plus className="w-3.5 h-3.5" /> Build {tb}</button>}
                  </div>
                        ...
                        <tbody>
                          {pj.map(p => (
                            <tr key={p.id} className="border-b border-[#231C30]/50 text-white">
                              <td className="py-2.5 text-[11px] md:text-sm pr-4 truncate max-w-[150px]">{p.name}</td>
                              <td className="py-2.5">
                                <select disabled={rl === "viewer"} value={p.status} onChange={async (e) => {
                                  ...
                                  await gd();
                                }} className="bg-[#171324] text-[11px] md:text-xs border border-[#362B4C] px-1.5 py-1 rounded text-white disabled:opacity-50">
                        ...
                        <tbody>
                          {tk.map(t => (
                            <tr key={t.id} className="border-b border-[#231C30]/50 text-white">
                              <td className="py-2.5 text-[11px] md:text-sm pr-4 truncate max-w-[150px]">{t.title}</td>
                              <td className="py-2.5">
      ...
      {cf && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-[60] p-4">
          <div className="bg-[#110E1A] border border-rose-500/20 rounded-xl max-w-sm w-full p-6 mx-4">
      ...
      {md && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="bg-[#110E1A] border border-[#231C30] rounded-xl max-w-md w-full overflow-hidden mx-4">
            <header className="p-4 border-b border-[#231C30] flex items-center justify-between bg-[#171324]">
              <h3 className="font-bold text-white text-sm uppercase">Initialize Node</h3>
              <button onClick={() => setMd(null)} className="text-[#565D7A] hover:text-white"><X className="w-4 h-4" /></button>
            </header>
            {md === "proj" ? (
              <form onSubmit={ap} className="p-4 md:p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 md:gap-4 pt-2"><button type="button" onClick={() => sbp("proj")} className="bg-[#171324] border border-[#362B4C] text-[11px] md:text-xs text-[#A29DB8] py-2 rounded-lg flex items-center justify-center gap-1"><Copy className="w-3.5 h-3.5" /> Save Template</button><button type="submit" className="bg-[#7C3AED] text-white text-[11px] md:text-xs py-2 rounded-lg">Commit</button></div>
              </form>
            ) : (
              <form onSubmit={at} className="p-4 md:p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 md:gap-4 pt-2"><button type="button" onClick={() => sbp("task")} className="bg-[#171324] border border-[#362B4C] text-[11px] md:text-xs text-[#A29DB8] py-2 rounded-lg flex items-center justify-center gap-1"><Copy className="w-3.5 h-3.5" /> Save Template</button><button type="submit" className="bg-[#7C3AED] text-white text-[11px] md:text-xs py-2 rounded-lg">Commit</button></div>
              </form>
            )}
          </div>
        </div>
